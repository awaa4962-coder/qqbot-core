...
evidenceIds 最多24项，每个 E 编号必须存在于下方给定材料中；headline 的编号必须来自已选 topic 引用过的证据。不要编造编号，不要把示例当成真实结论。
...
4. 正文和标题不要留下“今天、昨天、明天、今晚、明晚”：本日报今天=${options.dateText}，昨天=${previousDate}，明天=${nextDate}，按此写明确月日。不得把目标、估计或计划写成已完成。`;
}
