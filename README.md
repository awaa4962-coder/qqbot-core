...
`1.4.23-usage-dimensions` 是 API 用量维度的 WIP：按物理 HTTP 尝试归一化多协议 usage，管理员页面只读查看；缺报和历史未知值不会冒充零，不展示价格或节省金额。当前未声称候选验收或部署。筛选、隐私、故障与回滚限制见 [Linux 用量说明](deploy/linux/USAGE.md)。
