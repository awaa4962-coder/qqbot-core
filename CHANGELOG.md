...
- 修复“提示词要求合并同一话题，校验器却只允许单一证据片段”的协议冲突。topic 通过 `sourceDiscussionIds` 显式声明来源，所有来源及证据必须存在，每个来源都要实际引用；未声明跨片段引用和编造证据仍拒绝，旧单来源草稿兼容。
...
- 新增 15 项回归覆盖合并来源、未知证据、旧格式、标题证据、重写范围、提示词、完整/超量取材、纠正尾句、主备行为、失败不外发不落账及保稿。模型配置、认证、关系评分、日报白名单、发送去重不变，Windows 保持冻结。
- 本地验收：`npm ci` 通过，lint 0 errors / 0 warnings，757/757 测试、13/13 离线回放与发布检查通过，npm 审计 0 项已知漏洞。真实模型质量验收另行记录，单元测试通过不代表所有聊天语义都能正确理解。
