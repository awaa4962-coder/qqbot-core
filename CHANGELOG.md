...
- 新增 16 项回归：旧依赖/锁文件不一致拒绝、生产环境省略开发依赖、PNG/JPEG/WebP/GIF/AVIF 指纹与尺寸、双帧 GIF 分类、坏图片拒绝及词云真实 PNG 非空检查。使用合成图片和替代视觉调用，不读取真实群图片、不调用模型、不发送 QQ 测试消息。
...
- 本地验收：`npm ci` 通过，lint 0 errors / 0 warnings，741/741 测试通过，13/13 离线回放通过，发布检查通过；截至本轮 `npm audit` 报告 0 项已知漏洞。部署仍需在候选及运行容器内复核实际依赖，不能把该审计结果理解为整个系统不存在未知漏洞。
