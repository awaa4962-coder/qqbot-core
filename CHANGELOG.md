...
计划目标为 **v2.0.0**：用户指定本轮 P0-P5 智能化与模块化改造完成、整体验收通过后，正式版本统一为 `2.0.0`；目前尚未发布此版本。

最终同步源码版本、锁文件、版本与更新命令、中英文说明、前端、服务器实际运行及 GitHub。当前线上版本仍以各已部署批次记录为准，不因本条规划变更而重启或改号。

## v1.4.25-context-groups - 2026-09-23 - Linux 完整来源上下文
- 历史原话逐条携带作者、时间、来源与引用关系；实际提供的引用链作为完整组选择，不按字符预算拆断。来源与依赖超限时舍弃整组，不把正文留下却丢掉出处。
- 存档继续使用原有群500/用户300字符上限，新增脱敏后的原长度和截短标记。旧记录或矛盾字段保留“完整性未知”；当前摘录截短单独标明，只读召回同样区分存档完整性与摘录省略。
- 私聊文本附件不再拼进1000字符的当前输入。最多读取原有3份、每份10000字节的支持格式，正文作为独立证据；当前问题保留，实际入选、未读与因预算省略的份数明确告知。没有新增文档格式或放宽URL权限。
- 工具续问在动态自身信息组装后复查24k字符预算，只舍弃允许移除的旧历史组，保护当前输入、引用、明确记忆、附件和原生工具调用/结果。拒绝通过extra覆盖上下文、模型或输出预算；不能再缩减时停止本次请求，保留原有受控备用路径。
- 诊断区分初选、续问删组和发送前选择；显示引用最多24条时注明仅为界面省略，不冒充API没有收到。附件只显示序号，完整性枚举不包含原文、文件名或路径；发送前选择不等于成功送达。
- 提示词chat-v11/interjection-v6明确缺失证据边界。不增加付费压缩、额外总结、模型写记忆、数据库或普通聊天最终答案缓存。JM、模型任务主备、思考档位、关系评分和Windows安装不变。
- 候选1389项测试：Linux1386通过/3项浏览器在本地通过，本地1387通过/2项Windows软链接在Linux通过，新增63项。lint0/0、npm ci审计0、回放13/13、Linux完整发布检查/隔离启动/JM与FS、484文件隐私扫描及1440/390/320实际管理页通过。无付费模型调用、无QQ测试消息；候选门禁通过不冒充真实模型整体质量通过。Linux上线状态见实施记录。
- Linux已部署 `d8da944`：GitHub CI、受限备份、健康/就绪、QQ、JM及新诊断资源通过，配置不变、日报计划一份。NapCat/Windows未更新，只重建Bridge；回滚目标1.4.24保留新运行数据。严格验收16/43=37.2%，本轮新增P3-07一项，2.0.0尚未完成。

## v1.4.24-memory-state - 2026-09-23 - Linux 明确记忆状态
- 明确记忆区分未分类、事实、历史事件、待办与当前状态；来源类型仍区分用户命令和管理员备注，旧条目不推断类型。事件发生时间可空且不以来源时间代替；未来安排记录为待办。
- 新增分类记事与事项状态命令；待办使用 pending/in_progress/done/cancelled，当前状态使用 current/ended。状态转换只响应明确命令或控制台操作，不改正文或续期；过期表示未知，不表示状态反转。
- 控制台支持分类、事件时间和独立状态提交。管理员编辑仍记为 operator_note；“已完成”只是操作者声明，不代表机器人已执行。正文纠正沿用原 ID，不按标题自动合并。
- 聊天与只读召回接收结构化字段，提示词版本为 chat-v10。事项状态和记录有效性分开，不把记录冒充实时检测。引用已纠正或删除的原消息时拒绝；缺少完整依赖版本标记的旧线程保守弃用，新记忆命令不混入普通历史。偏好与模型推测保持独立；关系评分、JM、模型路由/思考档位、权限与私聊自动长期保存策略不变。
- 依赖随实际入选回合继续传递，诊断来源显示截断不再丢失依赖；更正或删除后旧转述也退出。后台画像生成增加全局隐私代次复查，聊天之外的旧异步结果也不能回填。普通正文纠正默认保留到期时间，过期须显式续期或重新记住。
- 回滚限制：旧版可读新字段，但旧编辑路径可能丢失分类。存在 typed 新数据时不得直接启用旧写路径；须先有兼容补丁，或保持 Bridge 停用新写路径等待修复。不得用旧备份覆盖新数据。
- 候选验收：1326项测试，Linux1323通过/3项浏览器专项在本地通过；本地1324通过/2项Windows软链接专项在Linux通过，新增33项。npm ci/审计0、lint0/0、回放13/13、隔离启动鉴权、完整Linux发布检查、JM/FS合成ZIP及474文件发布扫描通过，实际管理API的1440/390/320布局与状态编辑通过。
- 实际模型共6次：初候选待办边界通过，结束状态出现误判后增加记录有效性说明；最终仅复查结束状态，主备均不再把记录当实时检查。不同候选结果分别保留，未宣称最终全场景、整体智能或节费收益通过；自然度和既有图片推断缺陷仍待P5。部署证据见实施记录。
- Linux已部署 `0ea21fd`，CI、受限备份、健康/就绪、QQ、JM和新记忆接口通过。配置与定时任务保留，NapCat/Windows未更新；新字段存在时禁止裸回滚旧写器。严格验收进度15/43=34.9%，本轮新增P3-01一项。

## v1.4.23-usage-dimensions - 2026-09-23 - API 用量维度
- API 网关按每次物理 HTTP 尝试记录实际配置模型、任务、primary/fallback、提示词版本与指纹，以及 configured/effective reasoning mode；无法从旧数据确认的任务、模型、版本或模式保持 unknown，不推造历史归属。
- 多协议 usage 归一化为 input、output、reasoning、total 独立维度及各自报告标志。供应商未提供 usage 时不补零；缓存权重只使用已测 input token。未读全、部分读取、缺报和旧格式会明确保持不确定，不等同真实零；旧记录的正数用量继续有效，零值不作确认。
- 管理员现有 API 页面提供只读用量查看，可按 1/7/30 天、提供方、模型、任务、主备、提示词和实际模式筛选；不显示价格、费用或节省估算。成员原有缓存命中命令保留。
- 个人用量以 HMAC 关联，查询最多30天，过期日文件在后续写入时滚动清理，不承诺停机后仍自动物理删除；不存请求正文、私有 reasoning、密钥或原始用户 ID。reset 与普通 usage 共用每日文件；forget 写入失败时操作返回失败，不假报遗忘成功。
- reset 与普通 usage 共用每日 `usage-YYYY-MM-DD.jsonl`：普通 usage 最多 16 MiB，另为 reset 预留 1 MiB，读取上限 17 MiB；reset 写入需 fsync 确认。两类容量均耗尽时 forget 明确失败并向调用方传播。
- 回滚兼容：旧版可读取原格式文件中的 reset 记录，无需保留新模块补丁；回滚会失去新版统计精度与容量保护，绝不回灌旧状态。详见 [Linux 用量说明](deploy/linux/USAGE.md)。
- 最终候选：Linux1291通过/2项浏览器专项另在本地通过，本地含浏览器1291通过/2项Windows软链接专项在Linux通过，总计1293项，新增58项。npm ci/审计0、lint0/0、13/13回放、隔离启动鉴权与完整Linux发布检查通过；JM真实依赖/FS合成ZIP及470公开文件发布扫描通过。管理员1440/390像素实际接口检查通过，没有付费模型或QQ测试发送。
- 独立审查修复了日志满容量、未换行短写残片、超界数值、旧零占位、坏元数据与未报告标志矛盾；清除标记在新版及1.4.22读取器重启后均有效。线上1669条旧记录只读兼容检查通过；具体切换证据见实施记录。此阶段性版本不是最终 `2.0.0`，没有宣称已测得冷/热缓存节费。
- 已部署Linux代码 `c3327e1`，CI、受限备份与切换后健康/就绪、QQ、JM、用量接口均通过，原配置与定时任务保持，NapCat和Windows未更新。P0-P5严格验收进度14/43=32.6%，本轮新增P4-02一项。

## v1.4.22-vision-context - 2026-09-23 - Linux 图片语境
- 聊天插槽有视觉能力时，直接接收规范化图片和已选对话；文字插槽才按原视觉任务主备生成客观描述。主备本轮共用一次下载，备用不接收主模型推理或猜测，不改实际配置与思考档位。
- 图片来源区分当前、核验引用与明确指向的近期原消息；近期图片过滤遗忘/撤回/纠正和未来时间，原消息进入上下文。可见画面与语境推断分开，不凭空认人、补写失败图片或动态缺失帧。
- 图片最多三张、单图源文件 10 MiB/4000 万像素、最长边 2048，JPEG 单图/累计 1.5/4.5 MiB。SVG/PDF 拒绝，动态图明确仅首帧。图片字节独立计量但不豁免文本、工具和原生续接预算。
- 旧全局相似指纹描述退出聊天；新客观描述缓存按确切字节、用户/会话、模型和提示词版本隔离，内存最多 128 条/128 KiB、十分钟绝对期限，遗忘与修订清空。不得把模型推测写回长期事实，缓存不保存语境回复、图片或 URL。
- 控制台诊断显示本轮看图路径、失败/超限/首帧数量，不展示图片内容和推理。隐私说明明确像素不经过文字脱敏、本地遗忘不能撤回供应商收到的图片。
- 单张下载自身超时降级为不可读，不误伤整轮主备；接口错误只保留 HTTP 与固定错误类别，不回显供应商原始错误正文。输出检查拦截图片编码及本轮输入编码长片段，避免误发给 QQ 或继续当视觉描述。
- 验收：Linux 1234 测试通过/1 项可选浏览器跳过，独立浏览器专项 14/14；lint0/0、npm ci/审计0、13/13回放、隔离启动鉴权、完整发布门禁与真实 JM/FS 合成压缩包通过。主备使用同一合成图片的四个最终语境样例中，MiMo 仍多猜发图者的安慰动机，列为 P5 质量问题；不宣称识梗已解决。三轮合计12次模型请求，没有 QQ 测试发送。最终部署记录见实施文档。
- JM、FS、白名单、关系评分、关系导出预留和 Windows 冻结不变，最终 2.0.0 尚未完成。

## v1.4.21-scoped-tools - 2026-09-23 - Linux 有界只读工具
- 新增按需查询本人当前会话记忆/原话、权限内功能状态的只读工具；发送人和群/私聊范围来自后端，模型不能指定他人或跨群，空结果不放宽范围。私聊只读明确条目，不增加私聊原文采集，不开放写入、管理员命令、服务器文件或系统执行。
- 群聊、私聊、文件和主备插槽复用同一工具回合执行器，替代 MiMo 独立多轮搜索、DeepSeek 隐式预搜索和额外搜索总结分支；DeepSeek 仍按现有任务路由兜底，不改模型实例和思考档位。失败后的备用仅收到有界查询结果，不收到主模型的内部推理或原生续接状态。
- 单次聊天工具流程最多 4 个模型回合、8 次模型 HTTP 尝试、4 次工具处理，90 秒期限；单请求消息/声明序列化上限 24000 字符，工具单次 2000/累计 6000 字符，最多请求 1536 完成 token/次。请求额度不是实际计费证明，视觉预处理等其他任务仍需 P4 整体预算收口。
- 搜索仅使用用户本条明确公开查询中的连续关键词，不从记忆、引用、附件或其他工具结果拼接；文件和插话不外部搜索。无原生工具的文本接口保留后端明确公开请求的兼容搜索，仍过同一权限和预算；固定搜索接口拒绝重定向并接入取消信号。
- 工具参数和批次有界，未知工具、畸形参数、空结果、失败和预算耗尽分别处理，完整保留调用/结果配对；OpenAI Responses 与 Anthropic 原生续接只留在同一个冻结模型插槽的临时请求内，签名不截断或当回复发送。不支持的原生类型明确失败，未新增 Gemini 工具协议。
- 接收模型响应最多 256 KiB、最终正文最多 6000 字符，超限不发送巨量消息且不重复超限请求；工具拒绝回执计入累计预算。搜索授权额外覆盖“不用搜索”“别查了”“不要查”和后置取消，不把肯定搜索后的无关分句发到搜索服务。
- 工具实际读取的记忆编号/修订进入已确认发送轮次的依赖，后续纠正/遗忘仍会失效；本轮重复查询可复用，状态查询不缓存，也不把主模型身份转给备用当现状。诊断显示中文工具名、结果和计数，不记参数、正文、私有推理或续接内容。
- 验收：Linux npm ci/audit0、lint0/0、1135 测试通过/1 项可选浏览器跳过，该浏览器专项独立 14/14 通过；13/13 回放、隔离启动鉴权、完整 release:check 与发布包检查通过。4 次真实合成模型请求中主备均完成记忆/状态工具往返，只返回正确正文，未发送 QQ。独立审查修复搜索撤销和输出预算边界。JM 依赖及 FS 合成压缩包通过；关系评分、导出预留、认证和 Windows 冻结不变，最终 2.0.0 仍待全路线完成。服务器部署记录见实施文档。

## v1.4.20-memory-evidence - 2026-09-23 - Linux 来源化明确记忆
- 新增本人记忆的查看/记住/纠正/删除命令，群聊沿用 @ 门禁，各群与私聊独立；私聊显式保存单条，不开启自动私聊历史采集。命令内容保留大小写和标点，消息编号来自真实事件，无法指定他人身份。
- 复用既有画像文件保存明确条目，自述与管理员备注区分，带来源、修订和独立到期时间；每人每范围 32 条、全局 2048 条，默认 30 天、最长 90 天，闲聊不续期。先确认持久化再报成功，旧修订和遗忘前快照不可覆盖新状态，损坏文件不重置。
- 普通聊天不再把旧规则画像当作事实；明确记忆按问题相关性检索，近期话题线索从已有本群原话及编号生成，复读不增加来源数，不另存一份群聊天库。被 @ 者只提供本群话题线索，不公开其个人备注或全局偏好。
- 纠正时替换旧正文，保留有界来源编号，不回填旧声明或纠正前的相关助手轮次；遗忘会清理各范围条目并阻止旧异步结果，过期不参与模型输入。原话引用仍表示历史发言，来源不等于事实验证。
- 审查复现并补齐命令等待/发送隐私边界：入口代次贯穿私聊预处理，写入前重新检查权限，发送各段和重试前复核；已发段无法撤回。会话轮次记录记忆依赖修订，纠正/删除使旧依赖失效；同群其他发言人与插话也排除被替代来源，不公开他人的备注。独立的无正文否定标记有容量上限，原话仍在历史时不会因第九次纠正或条目到期重新放行。
- Linux 控制台新增“记忆”页，按明确范围/QQ 查询和编辑；支持新建与修改区分、脏草稿离开确认、失败/未知/冲突反馈、来源和期限展示。保存由后端标记管理员备注，接口不接受伪造用户来源；诊断仅投影编号、修订与原因。
- 关系算法及旧统计输入保留，管理员旧画像清理命令仍只清理画像；JM、模型主备/思考档位、认证、逐跳 SSRF、推理隔离和关系导出预留态不变。Windows 不更新，最终版本仍待全路线验收后统一为 2.0.0。
- 候选门禁：npm ci / audit 0、lint 0/0，Linux 1070 项中 1069 通过、1 项可选浏览器用例单独执行；本地 1067 通过、另有 2 项 Windows 软链接权限跳过。浏览器独立 14/14，以及真实管理 API 的 1440/390 像素新建/修改/删除/冲突/私聊隔离验证通过；13/13 回放、隔离启动鉴权和完整 Linux release:check 通过。JM 实际依赖、大写 FS 合成压缩包可用，不下载漫画或向 QQ 测试发送。
- 实际合成模型调用共 6 次，原候选主/备均采用纠正后的系统信息、不泄漏另一范围内容，但出现自动记住的含混承诺；最终候选增加实际记忆读取状态与 chat-v4 / interjection-v4 边界，2 次复查改为当前范围缺信息的追问，不再声称已查询/保存。MiMo 措辞仍略偏条目管理口吻，不能据此宣称整体聪明或自然度已达标；最终全场景评价仍在 P5。
- 服务器旧画像文件仅做结构兼容检查，未打印内容；最终可部署证据与 GitHub/服务器切换另行记录。本批不等同于全部智能记忆目标完成。
- Linux 已部署代码 `f0cba00`，GitHub CI 通过；源码与验收候选一致，受限备份目录 0700/归档 0600。Bridge 重建后健康/就绪、QQ 登录、JM、发送状态及记忆页面资源通过；记忆接口拒绝无明确范围的读取。API 配置、私有 sidecar、旧梗库归档校验和不变，日报计划一份；NapCat 未重启，Windows 未更新。回滚保留新记忆字段、否定标记与发送账本，只切旧代码/镜像，不能把旧版当成具有新记忆保护的等价版本。

## v1.4.19-context-provenance - 2026-09-23 - Linux 引用来源与输入快照
- 新增上下文引用校验模块，核对 OneBot 引用消息编号、群范围、作者、时间及既有遗忘截止线。跨群、私聊、错号、来源不完整、已遗忘或读取期间失效的引用不进入正文和图片上下文；清理画像仍不同于“忘记我”，没有扩大画像清理的含义。
- 提示词区分当前发言人、引用作者和所问对象；同名按稳定用户 ID 识别，来源核验不等于内容属实。引用缺失时只回答当前能确定的内容，必要时自然地请补充原话，不暴露内部校验或隐私原因，不以附近聊天冒充引用。
- 当前输入由上下文装配一次，群聊主模型、DeepSeek 备用、私聊/文件和插话入口复用；历史兼容调用仍可自行构造。长消息/引用保留首尾和截短标记，引用层整体保留或舍弃，裁剪后不留下没有作者的孤立正文。固定提示词标记为 chat-v3 / interjection-v3，不改变实际模型路由和思考档位。
- 入口捕获隐私代次，群成员/引用异步读取后与聊天运行开始前复核；私聊文件读取结束也复核，避免清理后重新写回旧昵称和上下文。附件正文不再触发 DeepSeek 的隐式自动搜索，普通聊天搜索不变。群消息图片接收日志只记数量，私聊命令日志不再把未知回执说成已发出。
- Linux 诊断显示引用来源是否已核验、作者、时间与拒绝原因，不显示正文、图片链接或内部推理。JM 独立权限、FS、延迟清理及 1.4.18 发送账本保留，关系评分和导出预留态不变；Windows 继续冻结。
- 独立审查补充作者字段冲突保护：顶层与 sender 作者编号矛盾时不任选其一；相同编号的字符串/数字形式和仅 sender 提供编号仍兼容。旧私聊集成测试隔离单写者发送账本，消除并行测试互相覆盖，不放松运行时持久化失败保护。
- 候选通过 `npm ci`、lint 0/0、Linux 1011/1011 无跳过、13/13 离线回放、隔离启动鉴权、依赖和完整发布检查；本地 1009 通过、2 项 Windows 软链接权限跳过。三个新增保护用例先复现失败再修正。真实 JM 依赖及 FS 合成 ZIP 验证通过，无漫画下载或 QQ 测试发送。
- 真实管理路由的合成诊断在 1440/390 像素验证来源显示、筛选、脱敏与布局。两个虚构场景分别调用当前主/备模型，共 4 次：同名引用没有错归用户或把计划当完成，缺失引用均请求补原话；DeepSeek 追问略长。未读取生产聊天，未记录推理正文，实际思考档位保持不变。这不是整体语义质量或节费证明，本批仍不是最终 2.0.0。
- Linux 已部署代码 `440f8f5`，GitHub CI 通过，切换前受限备份完成。Bridge 重建后健康/就绪 200、QQ 登录、JM 与新诊断资源正常；发送账本仍可读写，API 配置、私有 sidecar 和旧梗库归档校验和不变、日报计划一份。NapCat 未重启、Windows 未更新；回滚保留新数据与发送账本，只切代码和镜像，不回灌旧聊天。

## v1.4.18-delivery-state - 2026-09-23 - Linux 聊天发送状态
- 普通群聊、插话、私聊和私聊文件按消息编号保存发送元数据；请求接纳与每次发送前进行原子持久化，分段回执累加确认/未知计数。状态不含正文、提示词、推理或明文 QQ/群/消息编号，不缓存待发正文。
- 重启后的处理中记录显示为中断、部分发送或未知；不自动重启模型或补发。已结束记录约 24 小时后由既有维护任务清理，未知/部分/中断保留待核实，总量上限 10000；容量或文件异常时普通聊天停止，不能删除旧证据绕过。
- 单一 OneBot 回执分类不再让 message_id 或 retcode=0 覆盖失败字段；异步 status/retcode=1、矛盾回执、畸形码与 HTTP 非成功均保留未知。只有确定拒绝才沿用有限重试。日报遇到未知不能当作失败恢复，JM 仅明确上传成功才报完成，未知仍保留 FS 压缩包约一天。
- 普通资源转发、NapCat 登录检查、引用与成员资料读取、表情管理及流式上传共用同一响应判断；失败数据不能当作引用/昵称使用，命令回复未确认时不写“机器人已发言”标记。资源上传未知只提示核实，不自动重发，保留原立即清理策略。
- 对照 NapCat v4.18.13 的流式上传实现，远端临时文件名加入独立随机编号，防止跨群同名文件覆盖与旧清理计时器误删新文件；QQ 中显示的文件名不变。合并回执提供的流 ID、大小和 SHA256 必须匹配，不把跨容器临时传输等同于 QQ 已收到文件。
- 入口阻止重播已处理聊天与过期/已遗忘事件进入观察和生成。命令通道不依赖聊天状态文件可读性，仍执行原权限检查；本批没有为 JM 或所有业务命令新增跨重启自动任务恢复。
- Linux 诊断页新增发送状态筛选与人工核实，刷新后保留结果；确认只改元数据，不补发、不解除事件去重、不把未知正文回填成成功记忆。隐私说明同步，忘记我解除发送人/会话索引但保留负向事件标记；状态清理异常如实提示，不妨碍原聊天记忆清理。
- 版本为阶段性交付，最终 P0-P5 验收完成后的版本仍为 `2.0.0`。JM 独立权限、FS/延迟清理、模型任务主备/思考档位、关系评分和 Windows 冻结保持不变；上线记录另行追加，不因候选通过宣称全路线完成。
- 候选验收：`npm ci`、lint 0 errors / 0 warnings，本地 990 项中 988 通过与 2 项 Windows 软链接权限跳过，Linux 990/990 无跳过、13/13 离线回放、隔离启动鉴权、依赖与完整发布检查通过。实际 JM 依赖及大写 FS 合成 ZIP 可解、小写 fs 拒绝；两份同名微型文件真实流式传到 NapCat 的不同临时路径，保留约 60 秒，不发 QQ 消息或下载漫画。实际配置只读挂载核对模型路线，前端 1440/390 像素验证筛选/核实/刷新；无真实模型调用。Windows 未安装额外 JM 依赖或启动正式 Bot。
- Linux 已部署代码提交 `5481c8a`，GitHub CI 通过；受限备份完成，Bridge 已重建，健康/就绪、QQ 登录、JM 与新诊断资源通过，持久化目录可写。实际 API 配置和私有 sidecar/旧梗库归档校验和不变、日报计划一份，NapCat 与 Windows 未重启或更新。近满容量同盘合成测试的四次落盘生命周期约 89–136ms，不代表模型端到端延迟；旧版发言不会伪造补入新发送账本。

## v1.4.17-prompt-layers - 2026-09-23 - Linux 提示词与完整轮次
- 聊天和插话的固定规则不再混入每轮氛围、随机猫娘表现和群设置；动态表达设置作为资料放在固定系统规则及权限内自身信息之后。MiMo/DeepSeek 入口复用同一装配器，工具后续轮次不重复添加动态层，不改变任务主备与思考档位。
- 明确区分助手建议、用户执行反馈和已确认结果；当前纠正优先，失败后不重复原样建议，缺少关键证据只问必要项，不编口令、参数、梗出处或执行成果。保留猫娘表达，但不替代具体回答。
- 短期历史按完整用户/助手轮次进入上下文预算，优先最新轮次；预算不足时不只留下单边建议。长摘录保持原有 320/480 字符上限，改保留首尾并标记截短来源，避免吞掉末尾纠正。兼容旧记录，私聊仍不默认长期持久化。
- 诊断增加 `chat-v2` / `interjection-v2`、固定系统前缀指纹、固定/动态表达/组装正文字数，不记录完整提示词、私有推理或图片 base64，不把字符数当真实 token 数。合成回放使用同一装配和完整轮次选择，不新增每轮意图或反思模型。
- 修正正常“请补充上下文，方便判断”的追问被误判为内部分析；上下文相关规则要求额外的自我计划/组织回复线索，仍拒绝显式思考段、内部自指规划、非正文推理字段与不闭合标签，未改成放行思维链。
- JM、模型路由、思考强度、关系评分、生命周期保护及 Windows 冻结不变。此为阶段性交付，最终正式版本仍按总路线完成后统一为 `2.0.0`。
- 验收：安装与 lint 零错误零告警，本地 957 通过与 2 项 Windows 软链接权限跳过；最终 Linux 候选 959/959、13/13 离线回放、隔离启动、JM 实际依赖/FS 加密包及发布检查通过。前端 1440/390 像素验证提示词元数据、筛选和布局；不显示原文或私有推理。
- 真实合成回放共 9 次请求（预算上限 10），不读取生产聊天、不向 QQ 发送。首轮暴露重复密码检查及追问误杀，修正后承接样例改为询问格式/解压软件，未知梗样例正常说明不确定、不编出处且不暗示检索。多人对象、称呼与表情语境在首轮单例中符合预期，后续 P5 仍需扩大验证；图像案例使用文字描述，不算真实识图测试。观察到输入 6347、缓存输入 2368、输出 1257 token（含推理 697），不同候选混合数据不能证明稳定省费或整体质量达标。
- Linux 已部署代码提交 `716c82f` 并通过 GitHub CI；受限备份完成，Bridge 已重建且健康/就绪、QQ 登录与 JM 检查通过。真实配置与旧归档校验和不变，日报计划一份，NapCat 未重启，Windows 保持冻结。

## v1.4.16-chat-lifecycle - 2026-09-23 - Linux 聊天生命周期保护
- 普通群聊、自动插话、私聊和私聊文件共享有界的聊天运行上下文，记录回复修订和内存隐私代次。在模型重试/备用、搜索工具、视觉后续轮次、每个发送分段与重试、缓存及成功记忆回写前重新检查；失效结果使用 cancelled，不当作主动沉默或普通模型失败。
- 群/私聊权限和黑名单变更使旧回复停止；显式称呼、风格及重置只使该用户的旧生成失效。由于群上下文可能含其他成员资料，任何用户/群记忆清理保守作废当前进行中的聊天，不自动重建或补发。
- 同一用户和会话的并发生成以新请求替代旧请求，并中止旧模型 HTTP 等待；不同群、私聊、不同用户分别隔离。最多 1000 个活跃运行，默认 180 秒后在下一检查点停止；保留既有单次 HTTP 超时。服务停止会中止当前聊天模型调用，不再接受新聊天运行。
- 不承诺撤回已经发给供应商的请求或已经送达 QQ 的分段。发送期间状态变化只阻止后续操作；超时/断连的回执仍保留未知，不按失败自动重发，也不把未完成的整段回复记录成用户已看过。前端新增“已停止”和“回执未知”筛选与明确原因。
- 遗忘期间返回的实际模型用量只保留无用户关联的计数，不复活用户缓存统计；搜索日志改记长度，不记录查询与结果片段。JM 独立权限、FS 和延迟清理、实际模型路由/思考档位、关系评分不变，Windows 继续冻结。
- 本批不改变 WebSocket 会话内的排队次序，不把尚未出队的新消息当作已处理；队列调度、完整对话状态和提示词质量改造仍待后续阶段。已发请求无法从供应商侧撤回，隐私变化在后续边界阻止继续传输与写回。
- 验收：安装、lint 零错误零告警，本地 946 通过和 2 项 Windows 软链接权限跳过；Linux 最终候选 948/948、13/13 离线回放、隔离启动鉴权、依赖与发布检查通过。实际配置只读检查受控自身信息、失效回复、JM 依赖及真实 FS 合成加密包。前端 1440/390 像素验证已停止/回执未知筛选、原因与无横向溢出。29 项新增测试覆盖并发中止、容量释放、遗忘与权限变化、晚到视觉/表情、分段及未知回执；未调用真实模型或发送 QQ 测试内容。
- Linux 已部署代码提交 `e909830`，GitHub CI 通过；受限备份完成，Bridge 重建后健康/就绪及 QQ 登录正常。JM 保留，配置和归档校验和不变，日报计划仍一份，NapCat 和 Windows 未重启或更新。

## v1.4.15-self-context - 2026-09-23 - Linux 受控自述与回复结果
- 群聊、自动插话、私聊和文件聊天在实际模型插槽解析后注入会话权限内的运行事实；搜索工具轮次及最终搜索摘要沿用同一授权范围，主备各自使用本轮请求模型标识。不提供源码、地址、路径、名单或密钥，不把配置就绪说成联网可用，不把功能清单当作执行回执。
- 自身信息复用能力目录，不另存能力清单；最多 1800 字符、无时间戳、不修改历史或工具消息。仅声明实际协议支持且本轮开放的工具；JM 状态只读缓存，不为每次聊天启动依赖检查。该预算独立于既有历史预算，总提示词精简留待 P2/P4。
- 模型结果区分 reply / silence / error，保留原文本接口兼容。严格的空 reply 插话表示主动沉默，不触发备用；无正文、仅私有推理、格式错误仍按原任务主备处理，两边失败不再随机补短句。
- 群内直接 @、私聊和私聊文件失败时只发固定提示，不记为成功对话，不触发表情或画像刷新。发送确认、部分发送和未知回执逻辑不变；诊断页可筛选主动不回复，显示安全模型标识、事实版本和能力数，不显示完整 prompt 或私有推理。
- 保留 JM 群/私聊独立白名单、大写 FS、确认上传和延迟清理；自动梗库仍退役。实际任务分配与思考档位、关系评分、鉴权和 SSRF 边界不变，Windows 安装保持冻结。本批不宣称语义理解、完整对话生命周期或缓存优化已完成。
- 验收：安装、lint 零错误零告警；本地 917 通过、2 项 Windows 软链接权限跳过，Linux 候选 919/919 通过、无跳过，13/13 离线回放、隔离启动鉴权与发布检查通过。真实配置只读验证各聊天插槽的请求模型、会话权限与 JM 状态，实际 7-Zip 合成包验证 FS 可解而 fs 不可解；未调用真实模型或向 QQ 发送测试内容。浏览器 1440/390 像素验证主动沉默筛选、失败提示元数据及布局，并修正诊断按钮在窄屏挤压换行。
- Linux 已部署代码提交 `2868c14` 并通过 CI；受限备份完成，Bridge 重新拉起且健康/就绪、QQ 登录和 JM 检查通过。配置与原归档校验和不变，日报计划仍一份，NapCat 未重启。

## v1.4.14-meme-retirement - 2026-09-23 - Linux 自动梗库退役
- 从启动、群消息观察、主动/被动上下文和退出保存移除自动梗库调用；不再学习、衰减、定时联网更新或注入旧梗义。旧 `QQBOT_MEME_*` 设置不能重新启用这些入口，图片语境缓存与表情指纹处理独立保留。
- 普通梗库命令返回停用说明，管理员旧启停命令继续检查权限但不修改数据；活跃能力和帮助不再推广文字梗库。管理写入接口保留鉴权并返回 HTTP 410，后台任务拒绝旧更新/查证请求，审计 CLI 只返回退役状态，不再读取聊天记录。
- 前端主导航移除梗库，维护页保留可搜索的只读归档；删除写入、联网查证和自动回填界面逻辑，旧任务不会恢复或回填归档。读取采用有大小限制的只读文件投影，不加载种子、不迁移、不改原文件，裁剪身份/来源原文并脱敏凭据。
- 保留旧数据及独立的访问受限备份；当前未删除历史数据格式与兼容 helper，后续清理需审查实际调用方和迁移对应测试，不让旧自动管线重回运行入口。
- JM 群聊/私聊、独立白名单、大写 FS、上传确认和延迟清理不变；模型调用与主备、思考档位、关系评分、鉴权和 URL 安全逻辑不变。Windows 安装继续冻结。
- 验收：本地 lint 0 errors / 0 warnings，901 项测试中 899 通过、2 项 Windows 软链接权限跳过；隔离 Linux 候选 901/901 测试、13/13 离线回放、启动鉴权冒烟及发布检查通过。真实配置只读挂载下验证旧开关无法重启管线、归档字节不变、JM 依赖可用，并用实际 7-Zip 对合成文件生成加密 ZIP，确认大写 FS 可解、小写 fs 被拒绝。浏览器验证桌面/手机只读归档、过滤、转义和 410 拒绝写入；未向 QQ 发送测试内容或调用模型。
- Linux 已部署代码提交 `e6ee0a6`，健康/就绪/QQ 登录与私聊 JM 帮助通过；旧写入和后台任务在实际服务上均被拒绝。部署另存受限旧词条原件，原文件与真实配置校验和不变；NapCat 未重启、日报定时任务仍只有一份。
## v1.4.13-capability-state - 2026-09-23 - Linux 能力状态第一批
- 聊天/视觉能力改用实际 API 任务配置的共享只读检查，分别查看群聊、私聊和视觉路由，不再根据旧 MiMo/DeepSeek 密钥字段推断，也不写死主备模型。该检查不调用模型，不把配置就绪当作接口联网成功。
- 能力目录增加已安装、启用、会话许可和检查状态；复用消息准入和私聊权限判断。管理员命令豁免仍不放开普通 AI 私聊，JM 仍使用独立群/私聊白名单。私聊表情无需附带群聊表情白名单。
- JM 能力采用已有依赖检查缓存，检查中、过期和依赖异常如实显示；保留下载、压缩、上传、FS 密码与延迟清理，不安装或修改 Windows JM 环境。
- 控制台能力页展示上述状态；Linux 自描述改为实际平台和浏览器控制台，启动说明核对 `/ready` 与 JM，日报诊断指向 Linux 唯一计划任务；明确手动有界模型回放和无模型离线检查的区别。
- 本批只推进 P1 的能力事实基础，不宣称聊天模型已经接入自身能力、明确结果契约或梗库已经退役；后续依照总路线继续。模型调用、认证构造、主备分配、思考档位、关系评分与真实配置不变。
- 验收：本地安装与 lint 零错误零告警，889 项测试中 888 通过、1 项 Windows 软链接权限跳过；隔离 Linux 候选 889/889 测试、13/13 离线回放、启动鉴权冒烟、实际 JM 依赖和发布检查通过。浏览器使用隔离配置与实际管理 API 验证 1440/390 像素页面、筛选和状态，不启动 Windows Bot、不读取真实聊天、不调用模型或发送 QQ 消息。

## 未发布 - 2026-09-23 - Linux 智能化改造规划
- 扩充 Linux 总路线与模块目标实现文档，明确 P0-P5 阶段、现有代码入口、能力与权限来源、对话结果契约、梗库退役、记忆纠正、提示词与缓存及前端验收。
- 保留历史交付记录，新增待实施任务清单、质量评价、灰度发布与回滚要求；README 和工作流增加文档入口。
- 用户授权按阶段持续执行且只更新 Linux，JM 下载明确保留。P0 完成线上核对、访问受限备份与隔离恢复读取；同版 Bridge 已拉起，NapCat 未重启，尚未部署新功能。
- Linux 基线 877/877 测试、13/13 离线回放、lint、隔离启动、JM 依赖与发布检查通过；3 次合成模型调用记录回答和用量，短句承接样例暴露重复步骤与无依据猜测，作为后续对照，不宣称质量已解决。
- 本提交不修改运行代码、真实配置、模型路由或版本号；P1-P5 的实现与验收结果按后续实际批次记录。
...
- 发送超时、断网或无法解析回执视为结果未知，不盲目重发；明确失败回执才允许有限重试。修复 900 字边界的标点溢出，为引用消息读取增加截止时间。
- 凭据规则脱敏覆盖入库、旧记忆加载、模型文本请求及日志；第三方画像/关系只取本群证据。遗忘清理遗漏的画像和昵称字段，用代次校验阻止旧画像、关系短评和日报在异步调用后回填；每次备用调用前再次核验。
- 画像过期不再被无关新消息复活，零置信度不会绕过门槛；明确的直白程度、表情和正式程度偏好进入提示词。同一证据不在个人检索和群背景重复注入。隐私说明如实标明有限脱敏聊天片段与备份边界。
- 私聊图片接入既有视觉管线，文件走独立任务，主备模型真正串联；未闭合推理块整体拒绝，不把后续内部段落误发。HTTP 200 非法 JSON 不算调用成功，修正多协议缓存用量统计和搜索输入角色。
- 禁止停用仍被任务引用的 API；损坏配置不再静默换回内置模型。控制台配置分开呈现已保存/已生效值，环境变量控制项只读；修复保存后回退、运行模块状态、私聊管理员诊断及历史版本前缀碰撞。
- 日报和成员总结保留状态反转后的再次确认；旧事件重投不能绕过遗忘。手动日期脚本需明确目标群，dry-run 不保存草稿、不发送、不落发送账本；仍允许调用模型生成预览。未知回执返回待核实，补发重试耗尽不伪报完成。
- 控制台表情采用同源带令牌请求与可回收 Blob 预览；梗查证不覆盖已换词条或人工编辑，日报保存使用开始编辑时的版本。日志刷新接入真实动作，后台查询短暂失败可重试并保留任务 ID，不重复提交业务。
- 表情总开关关闭时禁止采集，近似指纹不覆盖旧素材身份；候选过期及容量在分析前检查。人工停用/隔离的梗不被自动更新恢复，查询遵守群范围。JM 压缩有截止时间并持续排空子进程输出，今天词云按上海自然日统计。
- 发布包默认拒绝 Linux 运行目录、私有环境文件和源码软链接；测试子进程被信号终止不能误报通过。Docker 日志设轮转，应用日志风暴同时限制终端与文件，致命异常清理后以非零状态退出供服务拉起。
- 保留关系评分算法、DeepSeek 兜底、认证构造函数、关系导出预留态和服务器白名单；不删除兼容导出，不引入新总线或统一万能管线。
- 本地验收：`npm ci` 通过、npm 审计 0 项已知漏洞，lint 0 errors / 0 warnings；874 项测试中 873 通过、1 项因 Windows 文件软链接权限跳过（Linux 候选必须补验）。13/13 离线回放、隔离 HTTP/WS 启动鉴权及发布检查通过；Playwright 验证带令牌的真实位图预览、切页恢复、配置反馈与手机/桌面布局。测试未调用真实模型或向 QQ 发消息；语义质量及未知凭据格式仍需持续观察。

## v1.4.10-summary-evidence - 2026-09-14 - 日报取材与证据校验修复
- 修复“提示词要求合并同一话题，校验器却只允许单一证据片段”的协议冲突。模型只引用真实 E 证据编号，程序根据给定材料计算 `sourceDiscussionIds`，不要求模型重复维护两套索引；未知证据、未知来源、与实际证据不符的话题锚点及越出重写范围的引用仍拒绝。旧草稿兼容，多填但未引用的有效来源不进入保存结果。
- 独立 `evidence-budget.mjs` 替换仅选五个片段的取材限制：默认 36000 字符估算预算内保留完整有效文字，长条保留头尾；超量时按全日时间范围抽样并保留首尾，正文说明分析数量及截短情况。采集上限、白名单和保留期不变。
- 独立 `document.mjs` 校验结构与来源，原导出兼容。提示词示例使用本轮真实编号，区分计划、接近目标和已完成，禁止把“快要过万”写成“已破万”。证据合法不等同于语义完全正确。
- 主备均无法生成合格结构化正文时明确失败，不发送占位提要、不保存伪成功草稿或登记已发送；真正低消息量的本地说明和旧纯文本接口保留。日志仅记录供应商与固定失败原因，不输出正文、证据原文或私有推理。
- 工作台单讨论重写保留其全部声明来源，不扩大范围；失败保留原稿并显示任务失败。截断响应不作为合格日报。
- 将事实和日期核对放在证据之后再次强调：区分数字目标与实际数据、正在前往与已经到场、玩笑身份与具体经历；相对日期锚定日报发生日，不引入额外自动润色模型轮次。
- 新增 16 项回归覆盖合并来源、未知证据、旧格式、标题证据、重写范围、提示词、完整/超量取材、纠正尾句、主备行为、失败不外发不落账及保稿。模型配置、认证、关系评分、日报白名单、发送去重不变，Windows 保持冻结。
- 本地验收：`npm ci` 通过，lint 0 errors / 0 warnings，758/758 测试、13/13 离线回放与发布检查通过，npm 审计 0 项已知漏洞。真实模型质量验收另行记录，单元测试通过不代表所有聊天语义都能正确理解。

## v1.4.9-security-patch - 2026-09-13 - Linux 依赖安全补丁
- `sharp` 从 0.35.3 更新为 0.35.4，预编译图片库携带 libheif 1.23.2，修复 GHSA-rgj7-g3m4-5g8c 涉及的图片解码漏洞。同步更新 sharp 自身的平台二进制依赖，不改图片指纹算法、视觉模型调用、表情策略或词云排版。
- ESLint 间接依赖 `js-yaml` 从 4.3.1 更新为 4.3.2，修复 GHSA-2883-xcg3-v3hh 的 YAML 合并 CPU 消耗问题。保持开发依赖身份，不加入生产容器。
- 新增 `check:dependencies`：核对实际安装的 sharp 与锁文件一致，检查可用 libheif 解码器和 js-yaml 的安全版本下限。完整镜像与离线源码 overlay 构建均执行检查，旧图片依赖不能靠更新源码或锁文件伪装成已修复；依赖变化必须重新安装并构建镜像。
- 发布检查增加本地依赖核验，GitHub CI 增加 high/critical 级别 npm 审计门禁。已知安全下限检查不替代持续更新的漏洞审计。
- 新增 17 项回归：旧依赖/锁文件不一致拒绝、生产环境省略开发依赖、PNG/JPEG/WebP/GIF/AVIF 指纹与尺寸、双帧 GIF 分类、坏图片拒绝及词云真实 PNG 非空检查。使用合成图片和替代视觉调用，不读取真实群图片、不调用模型、不发送 QQ 测试消息。
- Docker Hub 不可达时可用 `Dockerfile.dependencies` 复用已验收的系统/Python 底座，但必须按新锁文件重新安装全部生产 npm 依赖，并校验 7-Zip 与实际图片库版本。它不是离线源码 overlay，也不宣称修复底座系统或 Python 的其他漏洞。
- 构建时显式将源码和依赖清单归属到 node 用户；依赖检查在实际非 root 运行账号下执行，拒绝安全归档权限导致的“root 构建通过、运行账号读不到锁文件”候选镜像。
- 本轮仅更新 Linux 发布线；Windows 安装、模型主备、白名单、聊天总结、日报发送状态与私有数据格式不变。部署前保留源码及服务器私有状态备份。
- 本地验收：`npm ci` 通过，lint 0 errors / 0 warnings，742/742 测试通过，13/13 离线回放通过，发布检查通过；截至本轮 `npm audit` 报告 0 项已知漏洞。部署仍需在候选及运行容器内复核实际依赖，不能把该审计结果理解为整个系统不存在未知漏洞。

## v1.4.8-member-summary - 2026-09-11 - 成员聊天总结
- 新增独立 `features/conversation-summary/`，支持群聊 `@机器人 总结我`、`总结 @成员`、`总结 @甲 @乙`、`分别总结 @甲 @乙`、`总结帮助`。普通成员可用，群聊必须 @机器人；私聊仅说明用法，不跨群查询。
- 默认最近两小时，可选今天、昨天、最近若干分钟/小时/天及有效日期，范围限今天及前六天，一次最多五人。使用真实 @ 的 QQ 身份选人，不按同名昵称猜人，也不接受 @全体。
- 程序只做本群与时间筛选、去重、引用/邻近背景补充和长度限制，将原话直接交给 API。提示词要求自然具体地讲事情，不堆分析套话，不推断人格、不编梗、不把建议写成已执行；相对日期以每条消息自身日期换算。
- 提示词以最终情况为主、必要经过为辅，避免逐句流水账、反复“表示/随后”以及无关的消息报时；附每个来源日期的昨天/今天/明天对照，保留约定的最终日期，没有新增自动润色请求。
- 各目标轮流获得输入预算，同一人复读折叠，不同人的相同发言分别保留。秒级时间相同的消息维持采集顺序，避免把后续纠正排到早先判断之前；纯图片、机器人回复、命令和已清理占位不作为目标发言。
- `conversationSummaryGroupWhitelist` 默认继承功能群，可通过 `.env_conversation_summary_groups` / `QQBOT_CONVERSATION_SUMMARY_GROUPS` 或前端独立配置，显式空列表关闭。不会改变日报白名单或新增长期采集。
- API 新增 `conversation_summary` 插槽，旧配置首次读取时继承当前日报主备与思考档位，不改写旧文件。可在 API 页独立调整；主模型无可用正文或被截断时使用备用，全部失败给出明确提示，推理字段不外发。
- 使用现有任务执行器，每群互斥、默认一分钟冷却，重复事件不重复调用。进度元数据可在 Linux「诊断」页查看，不保存原话和总结正文到任务账本；不写用户画像、关系分数或日报发送标记。
- 读取时处理隐私清理标记，API 主备切换前及每个发送分段前再次核对清理代次。结果未确认时停止，不自动整篇重发；分段沿用 900 字与 300ms 间隔，仅首段引用命令。
- 记录缺失、裁剪或采样会在正文后说明。旧按日记录只覆盖已采集群，其他群依赖有限滚动记录，不能补回没收到或已过期的历史。Windows 安装保持冻结。
- 验收：lint 0 errors / 0 warnings，725/725 测试通过，`release:check` 与 13/13 离线回放通过。Playwright 检查独立白名单、API 插槽、真实命令分发、任务刷新恢复及桌面/手机布局。开发中共做 10 次合成输入的真实模型调用，最终单人、多人及分别总结三个场景通过人工核对，未向真实群发送测试稿；这不保证所有输入的模型理解和文风都完全一致。依赖版本未改；本次 `npm ci` 成功，但 `npm audit` 报告现有 `sharp` 和 `js-yaml` 两项 high，需另做依赖安全补丁。
