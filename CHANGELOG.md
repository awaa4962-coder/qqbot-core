...
## v1.4.7-modular-runtime - 2026-09-08 - Linux 模块整理与任务状态
- Linux 控制台主文件按页面拆分到 `Web/pages/`，公共状态、操作反馈、外观与任务轮询位于 `Web/ui/`。使用 ES 模块显式导入，主文件仅保留导航、事件接线和初始化；现有界面布局不重做，Windows 安装不更新。
- 新增轻量 `tasks/runner.mjs`，日报、梗库联网更新/查证、表情同步/分析/检查/清理和诊断回放共用执行机制。旧管理接口保持可用；浏览器耗时操作通过受保护的 `/admin/tasks` 返回任务编号并查询进度。
- 执行器限制并发与同类重入，元数据持久化，不把输入正文、模型结果或密钥写进任务账本。结果仅在内存保留十分钟；重启后未完成任务标记中断，不自动重做。超时通知为“仍在收尾”，实际操作结束前不释放占用，也不伪称已取消。
- 前端刷新后接回原后台任务，查证结果可回填到对应词条；存在未保存修改时不覆盖。日报轮询共用工具，仍由日报自身发送锁和回执账本决定是否发送，普通任务执行器不接管外发重试。
- JM 分离命令准入、Python 运行与健康检查、压缩、临时目录、传输适配。群聊与私聊共用一个下载/校验/压缩/上传/清理流程；两种白名单、大写 FS 和约一天后删除保持不变。
- 新增公共 JSON 原子写入与防抖保存器，聊天记录、用户记录、画像及日报原子写入使用公共实现。异步写完后同步检查代次并提交，退出时同步存档可使旧快照失效；保存失败保留 dirty 状态及重试，保留各模块原有数据格式与保存周期。
- 画像拆为 `memory-profile/store`、`updates`、`query`、`presentation` 和常量；旧入口保留原导出，画像提取规则、TTL、关系分数及上下文选择不变。
- 旧 `clients/llm-client` 使用已有 API transport，保留参数与返回字段、token 字段和单次请求行为；认证 helper 不变，旧入口也拒绝不安全重定向和未明确允许的本地接口。
- 日报新旧格式改为两个展示策略，共用唯一的模型主备执行与降级流程。旧 prompt/fallback 的公开工具保留兼容，不直接删除旧测试或改变正式结构化日报的提示词及证据校验。
- 前端 JavaScript 加入 `npm run lint`；新增模块链接与静态资源门禁、任务超时/恢复/隔离、原子存档竞争及兼容接口安全测试。发布前保留源码与服务器私有状态备份，镜像验收通过后重建 Linux Bridge。
- 验收：`npm ci` 通过，lint 0 errors / 0 warnings，702/702 测试通过，`release:check` 通过，13/13 离线回放通过。Playwright 检查页面导航、API 新增防覆盖、任务途中刷新、查证回填、失败恢复和 390/768/1440 宽度；测试模型与发送器均为合成替身，不向真实群发测试消息。

## v1.4.6-summary-workbench - 2026-09-08 - Linux 日报工作台
- 日报新增独立按日 JSONL 采集，位于私有数据目录 `.qqfriend/summaries/`。仅采集日报白名单群，每群每天最多 10000 条 / 6 MiB，保留今天及此前六个自然日；过期补到的消息不冒充当天消息。
- 采集发生在聊天语义复读过滤前，沿用入口消息幂等；凭据、地址和长编号在落盘前脱敏，不保存图片 URL。优先使用事件时间，并保留收到时间；长文本有头尾保留和截短标记，容量及残缺 JSONL 会显示覆盖不足。
- 读取时合并按日记录与旧滚动记录并按消息编号去重。既往未采集内容无法补回，统计明确写为“已采集”，不声称拥有整个群的全天完整记录。
- 分析按引用链、相关内容和参与者组织讨论，不沿用实时聊天的短窗口。保留后续否定、纠正、独立确认；原始证据按讨论分配预算，避免统一截掉 220 字以后的结论。
- 正式日报采用结构化主/备用模型输出，校验字段、讨论归属和证据编号后由程序排版；结构无效时进入备用，再退回本地采集线索。编号校验不等同于语义事实验证，模型仍可能误解，需要结合证据回放评价。
- 提示词将今晚、明天等相对时间锚定到聊天日期，程序提供前后日期，要求正文改写为明确月日；避免昨日的日报隔天发送后把聚餐时间再推迟一天。
- Linux 网页新增“日报”：按群和日期生成草稿、显示任务进度、查看脱敏证据、保存全文修改为新版本、对照旧版及重写单个讨论。每群每天最多十个草稿版本，未完成发送的原稿优先保留；全文手工编辑后可从此前结构化版本继续做单讨论重写。单讨论重写会清掉可能过时的总括句。
- 统一定时、手动和启动补发的发送账本，保留旧版已发送标记兼容。每段外发前写入尝试状态，回执后登记进度；已确认失败的剩余分段可手动续发，未知结果需在群里核实后确认，不整篇自动重发。
- 发送锁取得后再次检查状态；恢复操作不会在未取得锁前删除另一发送任务的尝试标记。命令群内预览限当前群，跨群预览走受保护的控制台或管理员私聊，群内不再显示服务器文件路径。
- “忘记我”同步清理新增日报记录及相关草稿，并用清理代次阻止进行中的旧生成结果写回或发送。每日结果日志不再包含整篇日报和证据；新工作台接口沿用管理令牌与本机访问边界。
- Windows 安装继续冻结，Linux 更新前保留源码、私有数据及日报发送状态备份，随后更新并重建 Bridge。
- 验收：`npm ci` 通过，lint 0 errors / 0 warnings，686/686 测试通过，`release:check` 通过；13/13 离线对话回放通过。Playwright 检查日报进度、编辑、对照、单讨论重写、错误反馈及 390/768/1440 宽度；浏览器交互验收使用模拟模型和发送器，不向真实群发送测试稿。

## v1.4.5-context-selection - 2026-09-07 - Linux 上下文选择
- 新增独立 `context/relevance.mjs` 与 `context/conversation-selection.mjs`。使用 Node 内置中文分词和两侧概念匹配，修复旧检索把“下载/压缩包”映射为 `jm` 后仍只搜索历史字面 `jm` 的漏召回；时间新旧仅用于同分排序，不把无关新消息当成相关记忆。
- 个人历史继续限定当前发送人与当前群；常用消息特征通过弱引用缓存复用，编辑正文会重新计算，“忘记我”会清空特征缓存，不增加磁盘索引或模型请求。
- 群背景从最多扫描最近 120 条、30 分钟的记录中，按引用链、提及对象和词义相关选择最多 8 条。没有明确对象也没有相关匹配时才保留最多 4 条近期背景；引用或提及明确时不回退到其他人的无关闲聊。
- 群消息持久化新增既有 `replyToMessageId` 字段的入口接线，保留引用关系；引用内容附上原发送人身份。旧数据无引用字段时仍可按文本和时间检索。
- 短期线程只注入本轮相关回合及后续承接结果；明确换话题不带旧任务，引用其他人的消息不带当前用户的旧线程和个人历史。线程存储格式、过期时间、关系评分不变。
- 修复群 @ 回复无条件补取全群近五分钟图片：有当前/引用图片时不追加历史图片；仅明确看图时选本人或被提及者最近一条图片消息，最多三张，不混入其他人的图片。
- 上下文来源编号、类型及“引用链/关键词/同义表达/承接/最近背景”原因随预算裁剪后进入内存诊断。来源元数据不进入模型消息；已截短的来源层会标记裁剪，不展示真实聊天正文、昵称、图片 URL 或密钥。
- Linux 控制台显示来源，回放扩展为 11 个合成场景，其中新增样例实际调用个人历史召回及群话题选择函数。输入和隔离检查不等价于真实模型理解能力评测；DeepSeek 备用与私有推理隔离保持不变。
- 验收：`npm ci` 通过，lint 0 errors / 0 warnings，655/655 测试通过，`release:check` 通过；13/13 离线回放检查通过，Playwright 验证桌面与移动宽度下的来源显示、筛选、生成、基线及评价保存。

## v1.4.4-linux-diagnostics - 2026-09-07 - Linux 消息诊断与对话回放
- 更新主线转为 Linux，Windows 安装和桌面启动器暂停更新；本轮只部署服务器 Bridge。
- 新增独立 `bridge/diagnostics/`：真实消息经过准入、命令或插话路由、上下文预算、API 调用、输出清洗和发送时记录元数据，以异步上下文隔离并发群消息。
- 消息记录保留在内存，最多 300 条、24 小时，重启清空。只记录编号、状态、时长和数量，不采集聊天正文、昵称、图片地址、完整提示词、原始异常或推理内容。
- Linux 浏览器诊断页新增最近消息筛选和阶段时间线，区分未触发、处理中、已发送、未产生回复、失败和部分成功；发送重试成功按最终成功记录。
- 新增 8 个合成回放场景：短句承接、多人对象、称呼纠正、图片语境、未知梗、反讽、话题切换和视觉失败。共用现有提示词、上下文格式、预算和输出清洗函数，但不读取真实用户记忆，不代表线上历史召回或视觉识别质量已通过评测。
- 默认输入检查不调用模型；点击生成后仅对所选样例调用当前群聊主模型，必要时调用 DeepSeek 备用，固定省额度、禁用工具，不发送 QQ 消息。每日最多 20 次生成尝试，并发生成互斥。
- 合成样例的基线、候选正文和固定选项评价保存在服务器私有数据目录；新候选不会覆盖基线，失败不会覆盖已有结果，模型推理字段永不保存。页面显示模型、版本及输入指纹变化。
- 新增 `npm run replay:check`，Linux 分支接入现有 CI。验收：`npm ci` 通过，lint 0 errors / 0 warnings，633/633 测试通过，`release:check` 通过，10/10 离线回放检查通过；Playwright 验证 1440、768、390 像素宽度及筛选、错误反馈、忙碌状态、生成、基线和评价持久化。

## v1.4.3-prompt-cache - 2026-08-27 - Prompt Cache 命中统计
- 统一 API 网关开始采集供应商返回的缓存用量，兼容 DeepSeek `prompt_cache_hit_tokens`、MiMo `prompt_tokens_details.cached_tokens`、Responses `input_tokens_details.cached_tokens` 及对应未命中、输出和推理 token 字段。
- 新增普通用户命令 `@夜星 缓存命中`，私聊可直接发送 `缓存命中`；展示本人今天、近 7 天以及 MiMo/DeepSeek 分项命中率，不接受目标 QQ 参数，也不能查询其他群友。
- 群聊主答、DeepSeek 兜底、私聊和图片识别都会将用量归属到实际触发者；内部 `usageContext` 不会进入模型请求体。
- 统计采用按日 JSONL 追加写入，Bridge 与日报容器不会互相覆盖；只保存 token 数和加盐匿名用户键，不保存提示词、聊天内容、模型正文或群号，30 天后自动清理。
- `忘记我` 会写入个人统计重置点，旧计数立即不再展示；其他用户和全局统计不受影响。
- 聊天系统提示词重排为稳定规则在前、动态猫娘动作和群氛围在后，增加可缓存的公共前缀，同时保持原有身份、安全边界和回复语义。
- 不缓存普通聊天的最终答案，不复用用户之间的回复，不记录或外发 `reasoning_content`；日报恢复、DeepSeek 兜底、白名单与 Linux 隔离部署保持不变。
- 验收：`npm run lint` 0 errors / 0 warnings，`npm test` 613/613 pass；新增多协议用量归一化、用户隔离、匿名落盘、忘记重置、网关归属、群聊 mention 门控、私聊命令和稳定前缀测试。
