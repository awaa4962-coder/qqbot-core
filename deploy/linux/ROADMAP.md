...
本文描述分阶段改造目标，不代表全部功能已经实现或部署。用户已授权按目标持续执行且仅更新 Linux；P0 已完成，P1 能力状态和自动梗库退役已部署，其余 P1 目标和 P2-P5 尚未完成。实际发布以每批验收记录为准；服务器私有配置、模型路由和 Windows 安装均保持不变。
...
| P1 能力与事实 | 统一能力来源和结果语义，受控自描述，梗库退出活跃链路 | P0 | 进行中：能力状态与梗库退役已上线 | 帮助/前端/自述一致，停用不损坏其他模块 |
| P2 对话核心 | 对话状态、回复调度、分层提示词和明确失败路径 | P1 | 未开始 | 不串对象、不假成功，沉默不触发降级 |
| P3 记忆与工具 | 来源化记忆、按需检索、图片上下文和受限工具回路 | P2 | 未开始 | 不越权，纠正/遗忘/失败都可验证 |
| P4 成本与缓存 | 稳定前缀、本地必要缓存、重复计算合并、用量对照 | P3 | 未开始 | 实测收益且质量、隐私和新鲜度不退步 |
