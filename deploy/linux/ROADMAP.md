...
本文描述分阶段改造目标，不代表全部功能已经实现或部署。用户已授权按目标持续执行且仅更新 Linux；P0 已完成，P1 能力状态/自述/梗库退役和 P2 聊天生命周期保护已分批部署，其余目标尚未完成。实际发布以每批验收记录为准；服务器私有配置、模型路由和 Windows 安装均保持不变。
...
| P2 对话核心 | 对话状态、回复调度、分层提示词和明确失败路径 | P1 | 进行中：已上线失效回复保护，提示词/队列仍待改造 | 不串对象、不假成功，沉默不触发降级 |
